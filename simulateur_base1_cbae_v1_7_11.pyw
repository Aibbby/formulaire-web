...
def build_message_frame(
    ...
    if template_name == "TNA CREATE ACTIVE":
        ...
        status_marker = bytes.fromhex("0604") + expiry_yymm.encode("cp037") + bytes.fromhex("0801")
        status_marker_pos = payload_mutable.find(
            status_marker, token_ref_pos + len(token_ref_bytes)
        )
        if status_marker_pos < 0:
            raise ValueError("Position de DE123.68.08 introuvable dans le dataset 68.")
        status_pos = status_marker_pos + len(status_marker)
