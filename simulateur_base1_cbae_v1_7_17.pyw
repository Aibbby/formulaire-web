...
class AppSimulateurBase1(tk.Tk):
    def __init__(self) -> None:
        ...
        reference_timestamp = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
        self.token_reference_var = tk.StringVar(value=f"TokRefID{reference_timestamp}00")
        self.pan_reference_var = tk.StringVar(value=f"{reference_timestamp}0000")
    ...
    def generate_reference_ids() -> tuple[str, str]:
        ...
        # DE123.68.05 est fixe sur 24 caractères dans les modèles actuels.
        # Le suffixe 00 complète TokRefID + YYYYMMDDhhmmss (22 caractères).
        token_reference_id = f"TokRefID{timestamp}00"
        # DE123.68.0B est fixe sur 18 chiffres dans les modèles actuels.
        pan_reference_id = f"{timestamp}0000"
