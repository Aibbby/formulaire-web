...
def build_message_frame(
    ...
    if template_name == "TNA CREATE ACTIVE":
        ...
        while pos < len(dataset_value):
            ...
            if tag == 0x03:
                value = token_requester_id.encode("cp037")
            elif tag == 0x05:
                value = token_reference_id.encode("cp037")
            elif tag == 0x08:
                value = token_status.encode("cp037")
            elif tag == 0x0B:
                value = pan_reference_id.encode("cp037")
            if len(value) > 255:
...
class AppSimulateurBase1(tk.Tk):
    ...
    def trigger_send_message(self) -> None:
        ...
        try:
            token_reference_id, pan_reference_id = self.generate_reference_ids()
            if self.random_token_reference_var.get():
                self.token_reference_var.set(token_reference_id)
            if self.random_pan_reference_var.get():
                self.pan_reference_var.set(pan_reference_id)
