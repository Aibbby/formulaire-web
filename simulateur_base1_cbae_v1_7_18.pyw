...
def build_message_frame(
    ...
    if template_name == "TNA CREATE ACTIVE":
        ...
        # Le statut est positionné dans le dataset 68 juste après le TLV
        # d'expiration 06 04 AAMM et l'en-tête 08 01. On le localise
        # relativement au Token Reference ID pour éviter de confondre
        # le caractère EBCDIC A (C1) avec un autre A présent dans les
        # valeurs configurables.
        token_ref_bytes = token_reference_id.encode("cp037")
        token_ref_pos = payload_mutable.find(token_ref_bytes)
        if token_ref_pos < 0:
            raise ValueError("Token Reference ID introuvable pour DE123.68.08.")
        # La date d'expiration du token (tag 06) est indépendante de DE14.
        # On localise donc la séquence TLV 06 04 <4 octets> 08 01 sans utiliser
        # la date d'expiration de la carte saisie dans l'IHM.
        search_from = token_ref_pos + len(token_ref_bytes)
        expiration_tag_pos = payload_mutable.find(bytes.fromhex("0604"), search_from)
        if expiration_tag_pos < 0:
            raise ValueError("Tag DE123.68.06 introuvable dans le dataset 68.")

        status_tag_pos = expiration_tag_pos + 2 + 4
        if payload_mutable[status_tag_pos:status_tag_pos + 2] != bytes.fromhex("0801"):
            raise ValueError("Tag DE123.68.08 introuvable après DE123.68.06.")

        status_pos = status_tag_pos + 2
        if status_pos >= len(payload_mutable):
            raise ValueError("Position de DE123.68.08 hors limites.")
        payload_mutable[status_pos] = token_status.encode("cp037")[0]
...
class AppSimulateurBase1(tk.Tk):
    ...
    def generate_reference_ids() -> tuple[str, str]:
        ...
        # Les modèles BASE I actuels imposent 24 caractères pour DE123.68.05
        # et 18 chiffres pour DE123.68.0B. Les millisecondes sont intégrées
        # sans modifier les longueurs attendues par le serveur.
        token_reference_id = f"TokRefID{timestamp}{milliseconds[:2]}"
        pan_reference_id = f"{timestamp}{milliseconds}{now.microsecond % 10}"
