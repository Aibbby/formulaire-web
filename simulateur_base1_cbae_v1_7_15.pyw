...
class AppSimulateurBase1(tk.Tk):
    ...
    def _build_server_tab(self) -> None:
        ...
        pan_frame.pack(side="right", fill="y", anchor="ne")
    ...
    @staticmethod
    def generate_token_reference_id() -> str:
        # 24 caractères alphanumériques majuscules, compatibles EBCDIC.
        alphabet = string.ascii_uppercase + string.digits
        return "".join(random.SystemRandom().choice(alphabet) for _ in range(24))

    @staticmethod
    def generate_pan_reference_id() -> str:
        # 18 chiffres, avec un premier chiffre non nul.
        rng = random.SystemRandom()
        return rng.choice("123456789") + "".join(rng.choice(string.digits) for _ in range(17))
    ...
    def trigger_send_message(self) -> None:
        ...
        try:
            if self.random_token_reference_var.get():
                self.token_reference_var.set(self.generate_token_reference_id())
            if self.random_pan_reference_var.get():
                self.pan_reference_var.set(self.generate_pan_reference_id())
