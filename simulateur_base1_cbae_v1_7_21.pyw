...
def rebuild_de123_dataset68(
    payload: bytearray,
    token_requester_id: str,
    token_reference_id: str,
    pan_reference_id: str,
    token_status: str,
) -> dict[int, bytes]:
    """
    Localise le véritable Dataset 68 de DE123, reconstruit ses TLV et
    recalcule à la fois la longueur du dataset et celle de DE123.

    Cette méthode fonctionne pour TAR MAX, TAR MIN et TNA CREATE ACTIVE.
    Elle évite toute recherche sur les anciennes valeurs du modèle.
    """
    replacements = {
    ...
    required_tags = set(replacements)

    candidates: list[tuple[int, int, list[tuple[int, bytes]]]] = []
    for dataset_start in range(1, len(payload) - 2):
        if payload[dataset_start] != 0x68:
            continue
        dataset_length = int.from_bytes(
            payload[dataset_start + 1:dataset_start + 3], "big"
        )
        value_start = dataset_start + 3
        dataset_end = value_start + dataset_length
        if dataset_length <= 0 or dataset_end > len(payload):
            continue
        ...
        while pos < dataset_end:
            if pos + 2 > dataset_end:
                ...
                break
            tag = payload[pos]
            item_length = payload[pos + 1]
            item_start = pos + 2
            item_end = item_start + item_length
            if item_end > dataset_end:
                valid = False
                break
            items.append((tag, bytes(payload[item_start:item_end])))
            tags.add(tag)
            pos = item_end

        if valid and pos == dataset_end and required_tags.issubset(tags):
            # Dans les modèles actuels, DE123 contient uniquement le Dataset 68 :
            # son préfixe d'un octet vaut donc 3 + longueur du dataset.
            expected_de123_length = dataset_length + 3
            if payload[dataset_start - 1] == expected_de123_length:
                candidates.append((dataset_start, dataset_end, items))

    if not candidates:
        raise ValueError(
            "Dataset 68 de DE123 introuvable ou structure TLV incohérente."
        )
    if len(candidates) > 1:
        raise ValueError(
            "Plusieurs Dataset 68 compatibles ont été trouvés dans la trame."
        )

    dataset_start, dataset_end, items = candidates[0]
    rebuilt_items: list[bytes] = []
    verified_values: dict[int, bytes] = {}
    for tag, old_value in items:
        value = replacements.get(tag, old_value)
        if len(value) > 255:
            raise ValueError(f"Valeur trop longue pour DE123.68.{tag:02X}.")
        rebuilt_items.append(bytes((tag, len(value))) + value)
        verified_values[tag] = value

    rebuilt_value = b"".join(rebuilt_items)
    rebuilt_dataset = (
        bytes((0x68,))
        + len(rebuilt_value).to_bytes(2, "big")
        + rebuilt_value
    )
    new_de123_length = len(rebuilt_dataset)
    if new_de123_length > 255:
        raise ValueError("DE123 dépasse 255 octets.")

    de123_length_pos = dataset_start - 1
    payload[dataset_start:dataset_end] = rebuilt_dataset
    payload[de123_length_pos] = new_de123_length

    # Vérification sur les octets réellement reconstruits.
    verify_start = de123_length_pos + 1
    verify_dataset_length = int.from_bytes(
        payload[verify_start + 1:verify_start + 3], "big"
    )
    verify_end = verify_start + 3 + verify_dataset_length
    pos = verify_start + 3
    read_back: dict[int, bytes] = {}
    while pos < verify_end:
        tag = payload[pos]
        length = payload[pos + 1]
        value_start = pos + 2
        value_end = value_start + length
        if value_end > verify_end:
            raise ValueError("Dataset 68 tronqué après reconstruction.")
        read_back[tag] = bytes(payload[value_start:value_end])
        pos = value_end

    for tag, expected in replacements.items():
        if read_back.get(tag) != expected:
            raise ValueError(
                f"Échec de reconstruction de DE123.68.{tag:02X}."
            )
...
def build_message_frame(
    ...
    if template_name in ("TAR MAX", "TAR MIN"):
        ...
        # DE123 / Dataset 68 : reconstruction TLV complète.
        # Cette logique remplace les anciennes valeurs du modèle, quelles
        # qu'elles soient (notamment les valeurs "toktok..." de TAR MAX).
        rebuild_de123_dataset68(
            payload_mutable,
            token_requester_id,
            token_reference_id,
            pan_reference_id,
    ...
    if template_name == "TNA CREATE ACTIVE":
        ...
        # DE123 / Dataset 68 : même moteur TLV générique que pour les TAR.
        rebuild_de123_dataset68(
            payload_mutable,
            token_requester_id,
            token_reference_id,
