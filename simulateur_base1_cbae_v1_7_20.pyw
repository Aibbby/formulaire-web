...
MESSAGE_REASON_CODES = {
    "Token create : 3700": "3700",
    "Token deactivated : 3701": "3701",
    "Token suspend : 3702": "3702",
    "Token resume : 3703": "3703",
    "Device prov result : 3711": "3711",
    "OTP verification result : 3712": "3712",
    "Call centre activation result : 3713": "3713",
    "Mobile banking app activation : 3714": "3714",
    "Token expiry update : 3716": "3716",
    "3DS browser activation : 3717": "3717",
    "PAN expiry update : 3720": "3720",
    "PAN update : 3721": "3721",
}
...
def build_message_frame(
    ...
    if len(token_reference_id) not in {24, 25}:
        raise ValueError("Token Reference ID doit contenir 24 ou 25 caractères.")
    if len(pan_reference_id) not in {18, 25}:
        raise ValueError("PAN Reference ID doit contenir 18 ou 25 caractères.")
    ...
    if template_name in ("TAR MAX", "TAR MIN"):
        ...
        old_requester = "40010030273".encode("cp500")
        if payload_mutable.count(old_requester) == 1:
            _replace_unique_bytes(
                payload_mutable,
                old_requester,
                token_requester_id.encode("cp500"),
                "DE123.68.03",
            )

        old_token_reference = "DNITHE302435148332454797".encode("cp500")
        if payload_mutable.count(old_token_reference) == 1:
            _replace_unique_bytes(
                payload_mutable,
                old_token_reference,
                token_reference_id.encode("cp500"),
                "DE123.68.05",
            )

        old_pan_reference = "888824070711288320".encode("cp500")
        if payload_mutable.count(old_pan_reference) == 1:
            _replace_unique_bytes(
                payload_mutable,
                old_pan_reference,
                pan_reference_id.encode("cp500"),
                "DE123.68.0B",
            )
        ...
        if payload_mutable.count(old_token_reference) == 1:
            token_ref_pos = payload_mutable.find(token_reference_id.encode("cp500"))
            status_pos = token_ref_pos + len(token_reference_id.encode("cp500")) + 3
            if 0 <= status_pos < len(payload_mutable) and payload_mutable[status_pos] == 0xC1:
                payload_mutable[status_pos] = token_status.encode("cp500")[0]
    ...
    if template_name == "TNA CREATE ACTIVE":
        # Positions fixes avant DE123 dans le modèle BASE I.
        ...
        # DE63.03 - Message Reason Code.
        _replace_unique_bytes(
            payload_mutable, bytes.fromhex("3700"),
            digits_to_bcd(message_reason_code), "DE63.03"
        )

        # DE123 contient le dataset 68 sous la forme :
        #   68 + longueur sur 2 octets + suite de TLV (tag/longueur/valeur).
        # On reconstruit entièrement ce dataset pour autoriser des valeurs
        # de longueur différente, puis on recalcule DE123, H04 et le framing TCP.
        dataset_start = payload_mutable.find(bytes.fromhex("680072"))
        if dataset_start < 1:
            raise ValueError("Dataset 68 introuvable dans DE123.")

        old_dataset_length = int.from_bytes(
            payload_mutable[dataset_start + 1:dataset_start + 3], "big"
        )
        dataset_value_start = dataset_start + 3
        dataset_end = dataset_value_start + old_dataset_length
        if dataset_end > len(payload_mutable):
            raise ValueError("Longueur du dataset 68 incohérente.")

        dataset_value = bytes(payload_mutable[dataset_value_start:dataset_end])
        tlvs = []
        pos = 0
        while pos < len(dataset_value):
            if pos + 2 > len(dataset_value):
                raise ValueError("TLV incomplet dans le dataset 68.")
            tag = dataset_value[pos]
            length = dataset_value[pos + 1]
            value_start = pos + 2
            value_end = value_start + length
            if value_end > len(dataset_value):
                raise ValueError(f"Longueur TLV incohérente pour le tag {tag:02X}.")
            value = dataset_value[value_start:value_end]
            if tag == 0x03:
                value = token_requester_id.encode("cp500")
            elif tag == 0x05:
                value = token_reference_id.encode("cp500")
            elif tag == 0x08:
                value = token_status.encode("cp500")
            elif tag == 0x0B:
                value = pan_reference_id.encode("cp500")
            if len(value) > 255:
                raise ValueError(f"Valeur trop longue pour le tag {tag:02X}.")
            tlvs.append(bytes((tag, len(value))) + value)
            pos = value_end

        required_tags = {0x03, 0x05, 0x08, 0x0B}
        found_tags = {item[0] for item in tlvs}
        missing = required_tags - found_tags
        if missing:
            missing_text = ", ".join(f"{tag:02X}" for tag in sorted(missing))
            raise ValueError(f"Tag(s) manquant(s) dans le dataset 68 : {missing_text}.")

        rebuilt_value = b"".join(tlvs)
        rebuilt_dataset = bytes((0x68,)) + len(rebuilt_value).to_bytes(2, "big") + rebuilt_value

        # Le préfixe de longueur DE123 est l'octet qui précède le dataset 68.
        de123_length_pos = dataset_start - 1
        new_de123_length = len(rebuilt_dataset)
        if new_de123_length > 255:
            raise ValueError("DE123 dépasse la longueur maximale sur un octet.")

        payload_mutable[dataset_start:dataset_end] = rebuilt_dataset
        payload_mutable[de123_length_pos] = new_de123_length

        # H04 et le préfixe TCP doivent refléter la nouvelle longueur.
        payload_mutable[3:5] = len(payload_mutable).to_bytes(2, "big")
        final_frame = make_tcp_frame(bytes(payload_mutable))

        # Contrôle de non-régression : relire le dataset réellement envoyé et
        # vérifier que les quatre valeurs sont strictement identiques à l'IHM.
        verification_payload = final_frame[4:]
        verification_start = verification_payload.find(bytes.fromhex("6800"))
        if verification_start < 0:
            raise ValueError("Dataset 68 absent après reconstruction.")
        verification_length = int.from_bytes(
            verification_payload[verification_start + 1:verification_start + 3], "big"
        )
        verification_data = verification_payload[
            verification_start + 3:verification_start + 3 + verification_length
        ]
        verified = {}
        verification_pos = 0
        while verification_pos < len(verification_data):
            if verification_pos + 2 > len(verification_data):
                raise ValueError("Dataset 68 tronqué après reconstruction.")
            verification_tag = verification_data[verification_pos]
            verification_value_length = verification_data[verification_pos + 1]
            verification_value_start = verification_pos + 2
            verification_value_end = verification_value_start + verification_value_length
            if verification_value_end > len(verification_data):
                raise ValueError("Longueur TLV incorrecte après reconstruction.")
            verified[verification_tag] = verification_data[
                verification_value_start:verification_value_end
            ]
            verification_pos = verification_value_end

        expected_values = {
            0x03: token_requester_id.encode("cp500"),
            0x05: token_reference_id.encode("cp500"),
            0x08: token_status.encode("cp500"),
            0x0B: pan_reference_id.encode("cp500"),
        }
        for verification_tag, expected_value in expected_values.items():
            if verified.get(verification_tag) != expected_value:
                raise ValueError(
                    f"La valeur DE123.68.{verification_tag:02X} reconstruite ne correspond pas à la saisie."
                )
...
class AppSimulateurBase1(tk.Tk):
    def __init__(self) -> None:
        ...
        self.message_reason_var = tk.StringVar(value="Token create : 3700")
        self.token_requester_var = tk.StringVar(value="Apple Pay")
        initial_token_reference, initial_pan_reference = self.generate_reference_ids()
        self.token_reference_var = tk.StringVar(value=initial_token_reference)
        self.pan_reference_var = tk.StringVar(value=initial_pan_reference)
        self.random_token_reference_var = tk.BooleanVar(value=True)
        self.random_pan_reference_var = tk.BooleanVar(value=True)
    ...
    def _build_server_tab(self) -> None:
        top_row = ttk.Frame(self.server_tab)
        top_row.pack(fill="x", pady=(0, 10))

        connection = ttk.LabelFrame(top_row, text="Connexion TCP", padding=10)
        connection.pack(side="left", fill="y", padx=(0, 10))
        ...
        pan_frame = ttk.LabelFrame(top_row, text="PAN", padding=10)
        pan_frame.pack(side="left", fill="y", anchor="n")

        ttk.Label(pan_frame, text="PAN :").grid(row=0, column=0, sticky="w")
        ...
        self.pan_entry.grid(row=0, column=1, padx=(5, 12), ipady=3, sticky="w")
        self.pan_entry.bind("<Double-Button-1>", lambda _event: self.open_card_selector())

        ttk.Label(pan_frame, text="Expiration AAMM :").grid(row=0, column=2, sticky="w")
        ...
        self.expiry_entry.grid(row=0, column=3, padx=(5, 0), ipady=3, sticky="w")

        self.tnr_options_frame = ttk.LabelFrame(
            self.server_tab, text="Paramètres messages", padding=10
        )
        self.tnr_options_frame.pack(fill="x", pady=(0, 10))

        ttk.Label(self.tnr_options_frame, text="Message :").grid(row=0, column=0, sticky="w")
        ...
        self.message_type_combo.grid(row=0, column=1, padx=(5, 10), sticky="w")
        ...
        self.tnr_options_frame.columnconfigure(5, weight=1)
        self.send_message_button.grid(row=0, column=5, padx=(18, 0), sticky="e")

        ttk.Label(self.tnr_options_frame, text="Message Reason Code :").grid(row=0, column=3, sticky="w")
        ttk.Combobox(
            self.tnr_options_frame,
        ...
        ttk.Label(self.tnr_options_frame, text="Token Requester ID :").grid(row=1, column=0, pady=(10, 0), sticky="w")
        ttk.Combobox(
            self.tnr_options_frame,
        ...
        ttk.Label(self.tnr_options_frame, text="Statut :").grid(row=1, column=3, pady=(10, 0), sticky="w")
        ttk.Combobox(
            self.tnr_options_frame,
            ...
            state="readonly",
            width=13,
        ).grid(row=1, column=4, padx=(5, 18), pady=(10, 0), sticky="w")

        ttk.Label(self.tnr_options_frame, text="Token Reference ID :").grid(row=2, column=0, pady=(10, 0), sticky="w")
        token_ref_frame = ttk.Frame(self.tnr_options_frame)
        token_ref_frame.grid(row=2, column=1, padx=(5, 18), pady=(10, 0), sticky="w")
        tk.Entry(
            token_ref_frame,
        ...
        ttk.Checkbutton(
            token_ref_frame,
            text="Aléatoire",
            variable=self.random_token_reference_var,
        ).pack(side="left", padx=(8, 0))

        ttk.Label(self.tnr_options_frame, text="PAN Reference ID :").grid(row=2, column=3, pady=(10, 0), sticky="w")
        pan_ref_frame = ttk.Frame(self.tnr_options_frame)
        pan_ref_frame.grid(row=2, column=4, padx=(5, 18), pady=(10, 0), sticky="w")
        tk.Entry(
            pan_ref_frame,
        ...
        ttk.Checkbutton(
            pan_ref_frame,
            text="Aléatoire",
            variable=self.random_pan_reference_var,
        ).pack(side="left", padx=(8, 0))
    ...
    def update_counters(self) -> None:
        self.counter_var.set(
            f"RX : 0800={self.count_0800}  0110={self.count_0110}  0630={self.count_0630}"
    ...
    def handle_received_frame(self, frame: bytes, payload: bytes) -> None:
        ...
        if parsed["mti"] == "0800":
            ...
            if response_code:
                self.log(f"Réponse TNA 0630 reçue — DE39={response_code}")
            else:
                self.log("Réponse TNA 0630 reçue.")

            self.after(0, lambda: self.send_message_button.configure(state="normal"))

        else:
            self.log(f"MTI {parsed['mti']} reçu — aucun traitement automatique nécessaire.")
    ...
    def on_message_type_changed(self, _event=None) -> None:
        # Les paramètres restent visibles pour tous les types de messages.
        return

    @staticmethod
    def generate_reference_ids() -> tuple[str, str]:
        now = datetime.datetime.now()
        timestamp = now.strftime("%Y%m%d%H%M%S")
        milliseconds = f"{now.microsecond // 1000:03d}"

        token_reference_id = f"TokRefID{timestamp}{milliseconds}"
        pan_reference_id = f"PanRefID{timestamp}{milliseconds}"
        return token_reference_id, pan_reference_id

    def append_message_log(self, metadata: dict) -> None:
        log_path = Path(__file__).resolve().parent / "simulateur_base1_cbae.log"
        now = datetime.datetime.now()
        header = (
            "Date;Heure;Pan;DatExp;Message;Message Reason Code;"
            "Token Requester ID;Statut;Token Ref ID;Pan Ref ID\n"
        )
        values = [
            now.strftime("%Y-%m-%d"),
            now.strftime("%H:%M:%S.%f")[:-3],
            str(metadata.get("pan", "")),
            str(metadata.get("expiry", "")),
            str(metadata.get("name", "")),
            str(metadata.get("message_reason_label", "")),
            str(metadata.get("token_requester_label", "")),
            str(metadata.get("token_status_label", "")),
            str(metadata.get("token_reference_id", "")),
            str(metadata.get("pan_reference_id", "")),
        ]
        line = ";".join(value.replace(";", ",").replace("\r", " ").replace("\n", " ") for value in values) + "\n"
    ...
    def trigger_send_message(self) -> None:
        ...
        try:
            # Les valeurs affichées dans l'IHM sont exactement celles envoyées.
            # Elles sont initialisées à l'ouverture et renouvelées après un envoi réussi.
            ...
            metadata.update({
                "message_reason_label": self.message_reason_var.get(),
                "token_requester_label": self.token_requester_var.get(),
                "token_status_label": self.token_status_var.get(),
                "token_reference_id": self.token_reference_var.get().strip(),
                "pan_reference_id": self.pan_reference_var.get().strip(),
            })
        except Exception as exc:
    ...
    def _send_message_worker(self, frame: bytes, metadata: dict) -> None:
        try:
            ...
            self.append_message_log(metadata)
            mti = metadata["mti"]
            if mti == "0100":
                self.count_0100 += 1
            elif mti == "0620":
            ...
            def prepare_next_references() -> None:
                next_token_reference_id, next_pan_reference_id = self.generate_reference_ids()
                if self.random_token_reference_var.get():
                    self.token_reference_var.set(next_token_reference_id)
                if self.random_pan_reference_var.get():
                    self.pan_reference_var.set(next_pan_reference_id)

            self.after(0, prepare_next_references)
        except Exception as exc:
